...
DATABASE_URL  = os.environ.get("DATABASE_URL", "postgresql://registros_db_br3e_user:9Q8kF2084mtEmOESc09jc22ZR7nS5FLz@dpg-d6ub6lfafjfc7380et2g-a/registros_db_br3e")
BANCA_INICIAL = 5000

CASAS = ["Bet365","Betano","SportingBet","Novibet","Vaidebet","Betfast","BETesporte",
         "Betao","Betnacional","BetFair","Investimento","Pagol","Vupi","Outra"]
...
def teclado_menu():
    return ReplyKeyboardMarkup([
        ["📝 Novo registro",   "⏳ Ver pendentes"],
        ["📈 Resultados",    "✏️ Editar registro"],
        ["📊 Gerar Dashboard"],
...
def teclado_resultados():
    return ReplyKeyboardMarkup([["🏦 Por Plataforma", "🔙 Voltar"]], resize_keyboard=True)
...
def inicializar_db():
    with conectar() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                ALTER TABLE registros ADD COLUMN IF NOT EXISTS freebet VARCHAR(3) DEFAULT 'nao'
            """)
        conn.commit()
...
def carregar():
    with conectar() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT * FROM registros ORDER BY data ASC, horario ASC, id ASC")
...
def inserir(a):
    with conectar() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO registros (data,horario,descricao,odd,stake,resultado,plataforma,esporte,freebet)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id
            """, (a["data"],a["horario"],a["descricao"],a["odd"],a["stake"],
                  a["resultado"],a["plataforma"],a["esporte"],a.get("freebet","nao")))
            new_id = cur.fetchone()[0]
        conn.commit()
    return new_id

def atualizar_campo(id_registro, campo, valor):
    if campo not in {"data","horario","descricao","odd","stake","resultado","plataforma","esporte"}:
        return
    with conectar() as conn:
        with conn.cursor() as cur:
            cur.execute(f"UPDATE registros SET {campo}=%s WHERE id=%s", (valor, id_registro))
        conn.commit()

def buscar_por_id(id_registro):
    with conectar() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT * FROM registros WHERE id=%s", (id_registro,))
            row = cur.fetchone()
    return dict(row) if row else None

# ── HELPERS ───────────────────────────────────────────────────────────────────
def lucro_registro(a):
...
async def menu_botao(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.get("modo") == "resultados":
        ...
        if txt == "🏦 Por Plataforma":
            return await resultados_por_plataforma(update, ctx)
        # Tenta como data
    ...
    if txt == "📝 Novo registro":     return await nova_registro_inicio(update, ctx)
    if txt == "⏳ Ver pendentes":   return await ver_pendentes(update, ctx)
    if txt == "📈 Resultados":      return await resultados(update, ctx)
    if txt == "✏️ Editar registro":   return await editar_inicio(update, ctx)
    if txt == "📊 Gerar Dashboard":  return await gerar_dashboard(update, ctx)

# ── NOVA APOSTA ───────────────────────────────────────────────────────────────
async def nova_registro_inicio(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
...
async def receber_odd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("💰 *Investimento (R$):*", reply_markup=teclado_cancelar(), parse_mode="Markdown")
        return STAKE
    except ValueError:
...
async def receber_stake(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Investimento inválido:")
        return STAKE
...
async def receber_esporte(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.pop("aguardando_esporte_custom", False):
        ...
        ctx.user_data["esporte"] = raw
    teclado_plataforma = [[c] for c in CASAS] + [[CANCELAR_BTN]]
    await update.message.reply_text("🏦 *Plataforma de registro:*",
        reply_markup=ReplyKeyboardMarkup(teclado_plataforma, resize_keyboard=True, one_time_keyboard=True),
        parse_mode="Markdown")
    return CASA

async def receber_plataforma(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    plataforma = update.message.text.strip()
    if plataforma == CANCELAR_BTN: return await cancelar(update, ctx)
    if plataforma == "Outra":
        await update.message.reply_text("Digite o nome da plataforma:", reply_markup=teclado_cancelar())
        ctx.user_data["aguardando_plataforma_custom"] = True
        return CASA
    ctx.user_data.pop("aguardando_plataforma_custom", False)
    ctx.user_data["plataforma"] = plataforma.strip().title() if plataforma not in CASAS else plataforma
    ...
    new_id = inserir({
        ...
        "plataforma": ctx.user_data["plataforma"], "esporte": ctx.user_data.get("esporte",""),
    })
    ...
    await update.message.reply_text(
        f"✅ *Registro #{new_id} salva!*\n\n"
        f"📅 {data_fmt}{hora_txt}\n🏅 {ctx.user_data.get('esporte','')}\n"
        f"🏷 {ctx.user_data['descricao']}\n🔢 Odd: {ctx.user_data['odd']}\n"
        f"💰 Investimento: R$ {float(ctx.user_data['stake']):.2f}\n🏦 Plataforma: {ctx.user_data['plataforma']}",
        parse_mode="Markdown"
    )
    return await voltar_menu(update, ctx)
...
async def ver_pendentes(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    registros   = carregar()
    pendentes = sorted([a for a in registros if a["resultado"] == "pendente"],
                       key=lambda a: (str(a["data"]), a.get("horario") or ""))
    if not pendentes:
        await update.message.reply_text("✅ Nenhuma registro pendente!")
        return
    linhas = [f"⏳ *{len(pendentes)} registros pendentes:*\n"]
    for a in pendentes:
        data_fmt = a["data"].strftime("%d/%m/%Y") if hasattr(a["data"],"strftime") else str(a["data"])[:10]
        hora = f" {a.get('horario','')}" if a.get("horario") else ""
        linhas.append(f"*#{a['id']}* {data_fmt}{hora} | odd {a['odd']} | R${float(a['stake']):.0f} | {a.get('plataforma','')}\n_{a['descricao']}_\n")
...
async def resultados(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    registros = carregar()
    res     = [a for a in registros if a["resultado"] in ("ganhou","perdeu")]
    if not res:
        await update.message.reply_text("Nenhuma registro resolvida ainda.")
        return
    lucro_total = sum(lucro_registro(a) for a in res)
    ...
    pendentes_ap = [a for a in registros if a["resultado"] == "pendente"]
    ...
    await update.message.reply_text(
        ...
        f"⏳ Investimento em curso: *R$ {stake_curso:.2f}* ({len(pendentes_ap)} registros)\n"
        f"📏 Unidades: *{lucro_total/50:+.2f}u*\n\n"
        f"📅 Digite uma data (DD/MM) para ver aquele dia\n"
        f"🏦 *Por Plataforma* para ver por plataforma\n"
        f"🔙 *Voltar* para sair",
        reply_markup=teclado_resultados(), parse_mode="Markdown"
    )

async def resultados_por_plataforma(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    registros = carregar()
    res     = [a for a in registros if a["resultado"] in ("ganhou","perdeu")]
    plataformas   = {}
    for a in res:
        plataforma = a.get("plataforma") or "Sem plataforma"
        if plataforma not in plataformas:
            plataformas[plataforma] = {"ap":0,"g":0,"stake":0.0,"lucro":0.0}
        c = plataformas[plataforma]; c["ap"] += 1; c["stake"] += float(a["stake"])
        if a["resultado"] == "ganhou":
            c["g"] += 1; c["lucro"] += float(a["stake"]) * (float(a["odd"])-1)
        else:
            c["lucro"] -= float(a["stake"])
    ordenadas = sorted(plataformas.items(), key=lambda x: x[1]["lucro"], reverse=True)
...
async def resultados_dia(update: Update, ctx: ContextTypes.DEFAULT_TYPE, raw: str) -> bool:
    ...
    registros  = carregar()
    do_dia   = [a for a in registros if str(a["data"])[:10] == data_str and a["resultado"] in ("ganhou","perdeu")]
    if not do_dia:
        await update.message.reply_text(
            f"Nenhuma registro resolvida em {data_obj.strftime('%d/%m/%Y')}.",
            reply_markup=teclado_resultados())
        return True
    lucro_dia = sum(lucro_registro(a) for a in do_dia)
    ...
    linhas.append("Digite outra data, 🏦 *Por Plataforma* ou 🔙 *Voltar*:")
    await update.message.reply_text("\n".join(linhas), reply_markup=teclado_resultados(), parse_mode="Markdown")
    return True

# ── EDITAR APOSTA ─────────────────────────────────────────────────────────────
CAMPOS_EDITAVEIS = ["data","horario","descricao","odd","stake","esporte","plataforma","resultado","cashout","freebet"]
CAMPOS_LABEL     = {
    "data":"📅 Data","horario":"⏰ Horário","descricao":"🏷 Descrição",
    "odd":"🔢 Odd","stake":"💰 Investimento","esporte":"🏅 Esporte",
    "plataforma":"🏦 Plataforma","resultado":"📊 Resultado","cashout":"💸 Cashout","freebet":"🎁 Freebet"
}
...
async def editar_inicio(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    pendentes = sorted([a for a in registros if a["resultado"] == "pendente"],
                       key=lambda a: (str(a["data"]), a.get("horario") or ""))
    linhas = ["✏️ *Editar registro*\n"]
    if pendentes:
        linhas.append("*Registros pendentes:*\n")
        for a in pendentes:
        ...
        linhas.append("Nenhuma registro pendente.\n")
    linhas.append("Digite o *ID* da registro que quer editar:")
...
async def editar_receber_id(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return EDITAR_ID
    registro = buscar_por_id(id_alvo)
    if not registro:
        await update.message.reply_text("❌ ID não encontrado.")
        return EDITAR_ID
    ctx.user_data["editar_id"] = id_alvo
    data_fmt = registro["data"].strftime("%d/%m/%Y") if hasattr(registro["data"],"strftime") else str(registro["data"])[:10]
    hora = f" {registro.get('horario','')}" if registro.get("horario") else ""
    info = (f"✏️ *Registro #{id_alvo}*\n\n📅 {data_fmt}{hora}\n🏷 {registro['descricao']}\n"
            f"🔢 Odd: {registro['odd']}\n💰 Investimento: R$ {float(registro['stake']):.2f}\n"
            f"🏦 Plataforma: {registro.get('plataforma','')}\n📊 Resultado: {registro['resultado']}\n\n*Qual campo editar?*")
...
async def editar_receber_campo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if campo == "plataforma":
        ...
        await update.message.reply_text("🏦 *Nova plataforma:*",
            reply_markup=ReplyKeyboardMarkup(teclado, resize_keyboard=True, one_time_keyboard=True),
            parse_mode="Markdown")
        return EDITAR_CASA
    if campo == "esporte":
...
async def editar_receber_plataforma(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    raw   = update.message.text.strip()
    if raw == CANCELAR_BTN: return await cancelar(update, ctx)
    campo   = ctx.user_data["editar_campo"]
    id_alvo = ctx.user_data["editar_id"]
...
async def aplicar_edicao(update, ctx, id_alvo, campo, novo_valor):
    ...
    if campo_real == "plataforma" and isinstance(novo_valor, str) and novo_valor not in CASAS:
    ...
    await update.message.reply_text(f"{emoji_conf} *Registro #{id_alvo} atualizada!*\n{label} → *{exibe}*", parse_mode="Markdown")
    return await voltar_menu(update, ctx)
...
async def gerar_dashboard(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    registros = carregar()
    registros_ord = sorted(registros, key=lambda a: (str(a["data"]), a.get("horario") or "99:99", int(a["id"])))
    df_res = [a for a in registros_ord if a["resultado"] in ("ganhou","perdeu")]
    ...
    # Métricas
    lucro_total = sum(
        lucro_registro(a) for a in df_res
    )
    ...
    stake_curso = sum(float(a["stake"]) for a in registros if a["resultado"]=="pendente")
    cards = [
    ...
    for i,a in enumerate(registros_ord):
        ...
        vals=[a["id"],data_fmt,a.get("horario",""),a["descricao"],a["odd"],a["stake"],
              a.get("esporte",""),a.get("plataforma",""),res_d,lucro_a,banca_a,prog_a]
    ...
    tr=DAT+len(registros_ord); ws.row_dimensions[tr].height=20
    for c in range(1,13):
    ...
    for c,h in enumerate(["Data","Registros","Lucro do Dia","Acumulado"],1):
    ...
    for i,(data_k,ap) in enumerate(sorted(por_dia.items())):
        ...
        lucro_d=sum(lucro_registro(a) for a in ap)
        acum3+=lucro_d
    ...
    for a in df_res:
        plataforma=a.get("plataforma") or "Sem plataforma"
        if plataforma not in plataformas: plataformas[plataforma]={"ap":0,"g":0,"stake":0.0,"lucro":0.0}
        c=plataformas[plataforma]; c["ap"]+=1; c["stake"]+=float(a["stake"])
        if a["resultado"]=="ganhou": c["g"]+=1; c["lucro"]+=lucro_registro(a)
        else: c["lucro"]+=lucro_registro(a)
    wc=wb.create_sheet("Por Plataforma")
    ...
    for c,h in enumerate(["Plataforma","Registros","Ganhou","Perdeu","Investimento","Lucro","ROI","Win Rate"],1):
        cell=wc.cell(row=3,column=c,value=h); est(cell,bold=True,bg=DARK,size=10); cell.border=brd()
    wc.row_dimensions[3].height=22
    for i,(nome,c) in enumerate(sorted(plataformas.items(),key=lambda x:-x[1]["lucro"])):
        er=4+i; rb=WHITE if i%2==0 else ALT
    ...
    for a in df_res:
        ...
        if a["resultado"]=="ganhou": e["g"]+=1; e["lucro"]+=lucro_registro(a)
        else: e["lucro"]+=lucro_registro(a)
    ...
    for c,h in enumerate(["Esporte","Registros","Ganhou","Perdeu","Investimento","Lucro","ROI","Win Rate"],1):
    ...
    for c,h in enumerate(["Semana","Periodo","Registros","Ganhou","Perdeu","Investimento","Lucro","ROI"],1):
    ...
    for i,((ano,num),ap) in enumerate(sorted(por_sem.items())):
        er=4+i; rb=WHITE if i%2==0 else ALT
        lucro_s=sum(lucro_registro(a) for a in ap)
    ...
    wg["A1"]="Registro #"; wg["B1"]="Banca Acumulada"
    ...
    if len(banca_acum)>=2:
        ...
        chart.y_axis.title="R$"; chart.x_axis.title="Registro #"
    ...
    await update.message.reply_document(
        ...
        caption=f"Dashboard gerado! {total} registros | R$ {lucro_total:.2f} lucro | ROI {roi:+.1%}"
    )
...
async def exportar_csv(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    registros   = carregar()
    total     = len(registros)
    pendentes = sum(1 for a in registros if a["resultado"] == "pendente")
    import io
    output = io.StringIO()
    import csv as csv_mod
    writer = csv_mod.writer(output, quoting=csv_mod.QUOTE_ALL)
    writer.writerow(["id","data","horario","descricao","odd","stake","resultado","plataforma","esporte"])
    for a in registros:
        data = a["data"].strftime("%Y-%m-%d") if hasattr(a["data"],"strftime") else str(a["data"])
        writer.writerow([a["id"],data,a.get("horario",""),a["descricao"],a["odd"],a["stake"],a["resultado"],a.get("plataforma",""),a.get("esporte","")])
    csv_bytes = output.getvalue().encode("utf-8")
    caption   = f"\U0001f4ca registros.csv\n{total} registros | {pendentes} pendentes"
    await update.message.reply_document(document=InputFile(csv_bytes, filename="registros.csv"), caption=caption)
...
def main():
    ...
    conv_nova = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^📝 Novo registro$"), nova_registro_inicio)],
        states={
            ...
            CASA:      [MessageHandler(filters.TEXT & ~filters.COMMAND, receber_plataforma)],
        },
    ...
    conv_editar = ConversationHandler(
        ...
        states={
            ...
            EDITAR_CASA:  [MessageHandler(filters.TEXT & ~filters.COMMAND, editar_receber_plataforma)],
        },
        fallbacks=fallbacks_padrao,
    )
